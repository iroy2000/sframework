...
class Config_Ini extends Config
{
    ...
    public function __construct($filename, $section = null, $config = false)
    {
        if (empty($filename)) {
            ...
            throw new Config_Exception('Filename is not set');
        }
        ...
        foreach ($iniArray as $key => $data)
        {
            ...
            switch (count($bits)) {
                ...
                default:
                    /** @see Zend_Config_Exception */
                    require_once 'zend/Exception.php';
                    throw new Config_Exception("Section '$thisSection' may not extend multiple sections in $filename");
            }
        }
        ...
        if (null === $section) {
            ...
            foreach ($section as $sectionName) {
                if (!isset($preProcessedArray[$sectionName])) {
                    /** @see Zend_Config_Exception */
                    require_once 'zend/Exception.php';
                    throw new Config_Exception("Section '$sectionName' cannot be found in $filename");
                }
            ...
            if (!isset($preProcessedArray[$section])) {
                /** @see Zend_Config_Exception */
                require_once 'zend/Exception.php';
                throw new Config_Exception("Section '$section' cannot be found in $filename");
            }
        ...
        foreach ($thisSection as $key => $value) {
            if (strtolower($key) == ';extends') {
                if (isset($iniArray[$value])) {
                    ...
                    /** @see Zend_Config_Exception */
                    require_once 'zend/Exception.php';
                    throw new Config_Exception("Section '$section' cannot be found");
                }
            } else {
        ...
        if (strpos($key, $this->_nestSeparator) !== false) {
            ...
            if (strlen($pieces[0]) && strlen($pieces[1])) {
                if (!isset($config[$pieces[0]])) {
                    ...
                    /** @see Zend_Config_Exception */
                    require_once 'zend/Exception.php';
                    throw new Config_Exception("Cannot create sub-key for '{$pieces[0]}' as key already exists");
                }
                $config[$pieces[0]] = $this->_processKey($config[$pieces[0]], $pieces[1], $value);
            } else {
                /** @see Zend_Config_Exception */
                require_once 'zend/Exception.php';
                throw new Config_Exception("Invalid key '$key'");
            }
        } else {
            $config[$key] = $value;
        }
